...
def resam_ts_data(
    ...
    if isinstance(feature.index, pd.MultiIndex):
        if callable(method):
            method_func = method
            return feature.groupby(level="instrument", observed=False).apply(method_func, **method_kwargs)
        elif isinstance(method, str):
            return getattr(feature.groupby(level="instrument", observed=False), method)(**method_kwargs)
    else:
        if callable(method):
            method_func = method
