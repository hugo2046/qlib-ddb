...
class KronosPredictor:
    ...
    def generate(self, x, x_stamp, y_stamp, pred_len, T, top_k, top_p, sample_count, verbose):
        ...
        y_stamp = y_stamp.to(self.device, dtype=torch.float32)
        
        preds = auto_regressive_inference(self.tokenizer, self.model, x, x_stamp, y_stamp, self.max_context, pred_len,
    ...
    def predict(self, df, x_timestamp, y_timestamp, pred_len, T=1.0, top_k=0, top_p=0.9, sample_count=1, verbose=True):
        ...
        x_np = df[self.price_cols + [self.vol_col, self.amt_vol]].values.astype(np.float32)
        ...
        pred_df = pd.DataFrame(preds.detach().cpu().numpy(), columns=self.price_cols + [self.vol_col, self.amt_vol], index=y_timestamp)
        return pred_df
